...
- (void)loadView
{
    ...
    self.mapShadowView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ShadowBottom"]] autorelease];
    ...
    headerView.image = [UIImage imageNamed:@"DetailHeader"];
    ...
    self.tableView.tableHeaderView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ShadowTop"]] autorelease];
    self.tableView.tableFooterView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ShadowBottom"]] autorelease];
    self.tableView.contentInset = UIEdgeInsetsMake( -20, 0, toolbarHeight - 20, 0 );

    [self.view addSubview:tableView];

    self.shadowView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ShadowBottom"]] autorelease];
    ...
    {
        self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:(mapHidden) ? @"ShowMap":@"HideMap"] style:UIBarButtonItemStyleBordered target:self action:@selector(toggleMap:)] autorelease];
    }
    ...
    csvItem.width = 40.0;
    self.prevItem = [[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Back"] style:UIBarButtonItemStylePlain target:self action:@selector(selectPreviousReport:)] autorelease];
    self.nextItem  = [[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Forward"] style:UIBarButtonItemStylePlain target:self action:@selector(selectNextReport:)] autorelease];
...
{
    if ( selectedReportIndex >= [reports count] - 1 )
    {
        return;
    }
...
- (void)updateHeader
{
    if ( viewMode == ReportDetailViewModeCountries )
    {
        ...
        else
        {
            ...
            self.headerIconView.image = [UIImage imageNamed:@"AllApps"];
    ...
    else
    {
        ...
        if ( self.selectedCountry )
        {
            self.headerIconView.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@", [self.selectedCountry lowercaseString]]];
        ...
        {
            self.headerIconView.image = [UIImage imageNamed:@"world"];
